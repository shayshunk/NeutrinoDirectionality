...
void AddPlotLabel(char const* label,
                  double const x,
                  double const y,
                  double const size = 0.05,
                  int const color = 1,
                  int const font = 62,
                  int const align = 22,
                  double const angle = 0);

void DecodePosition(string const& opts, double size, int& align, double& xLabel, double& yLabel);
...
void AddHistoTitle(char const* title, double titleSize, int titleFont)
{
...
void MultiPrint(TCanvas* c, string const print_topdir, string const& typeStr)
{
    ...
    int const oldVerbosity = gErrorIgnoreLevel;
...
// The chi2 could also be calculated as tmp_h_data_IBD->Chi2Test(tmp_h_realsim_IBD, "WW P CHI2"). "WW" when both histograms are
// weighted. See ROOT documentation https://root.cern.ch/doc/master/classTH1.html#ab7d63c7c177ccbf879b5dc31f2311b27
...
double Chi2DataMC(const TH1* dataHist, const TH1* mcHist, int& ndf, bool useOnlyShapeErrors)
{
    ...
    int const lowBin = tmpMC->GetXaxis()->GetFirst();
    int const highBin = tmpMC->GetXaxis()->GetLast();

    for (int i = lowBin; i <= highBin; ++i)
    {
        if (tmpData->GetBinError(i) * tmpData->GetBinError(i) + tmpMC->GetBinError(i) * tmpMC->GetBinError(i) > 0)
        {
            chi2 += (tmpData->GetBinContent(i) - tmpMC->GetBinContent(i)) * (tmpData->GetBinContent(i) - tmpMC->GetBinContent(i))
...
{
    int const highBin = h->GetNbinsX() + 1;
    int const lowBin = 0;
...
double Chi2DataMC_Test(const TH1D* dataHist, const TH1D* mcHist, int& ndf)
{
    ...
    {
        int const NbinsTotal = tmpMC->GetNbinsX() + 2;  //+2 for under/overflow
    ...
    // ROOT can't seem to handle small entries with lots of zeros? Suggested scaling the histogram and then rescaling the
    // inverted matrix
    // covMatrix *= 1e80;
    ...
    for (int i = lowBin; i <= highBin; ++i)
    {
        int const iErrBin = i - lowBin;
        double const x_data_i = tmpData->GetBinContent(i);
        double const x_mc_i = tmpMC->GetBinContent(i);

        for (int j = lowBin; j <= highBin; ++j)
        {
            int const jErrBin = j - lowBin;
            double const x_data_j = tmpData->GetBinContent(j);
            double const x_mc_j = tmpMC->GetBinContent(j);
            double const chi2_ij = (x_data_i - x_mc_i) * errorMatrix[iErrBin][jErrBin] * (x_data_j - x_mc_j);
            chi2 += chi2_ij;
        }
...
void DrawErrorMatrix(TMatrixD const& matrix, TAxis const* axis, double const maximum)
{
    ...
    else
    {
        ...
        {
            double const absmax = std::max(fabs(min), fabs(max));
...
void DecodePosition(string const& opts, double size, int& align, double& xLabel, double& yLabel)
{
    ...
    {
        string const opts1 = opts.substr(0, dashLoc);
        int align1;
        double x1, y1;
        DecodePosition(opts1, size, align1, x1, y1);

        string const opts2 = opts.substr(dashLoc + 1);
        int align2;
        double x2, y2;
    ...
    double const xLeft = gStyle->GetPadLeftMargin() + 0.03;
    double const xCenter = .5;
    double const xRight = 1 - gStyle->GetPadRightMargin() - 0.025;

    double const yBottom = gStyle->GetPadBottomMargin() + size / 2.;
    double const yCenter = .5;
    double const yTop = 1 - gStyle->GetPadTopMargin() - size / 2.;
...
void AddChi2Label(
    const TH1* dataHist, const TH1* mcHist, string const& opts, double size, double yOffset, bool useOnlyShapeErrors, bool useProb)
{
    int align;
