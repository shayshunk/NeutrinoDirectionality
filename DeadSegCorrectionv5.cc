...
int main()
{
	...
	//----------------------------------------------------
	{
		...
		for (int c = 1; c <= 5; c++)
		{
			...
			string file_list = Form("/project/prospect/tmp/Analyzed/Analyzed_2022_IBD_DS_SEER_Data/DS_SEER_Data_NewCal/SEER_DS_period_%s/Period_%s_files.txt", period.c_str(), period.c_str());
			// Opening File
			ifstream file;
			...
			while (file.good() && !file.eof())
			{
					...
					TString st = Form("/project/prospect/tmp/Analyzed/Analyzed_2022_IBD_DS_SEER_Data/DS_SEER_Data_NewCal/SEER_DS_period_%s/%s/AD1_IBD_2022_DS_SEER.root", period.c_str(), line.data()); // Yale Meitner server
	...
	sigmaBias[0] = sqrt(sigmaBias[0] * sigmaBias[0] + (0.25 * 0.25) + (0.08 * 0.08));
		sigmaBias[1] = sqrt(sigmaBias[1] * sigmaBias[1] + (0.39 * 0.39) + (0.08 * 0.08));
	sigmaBias[2] = sigmaBias[2] / sqrt(effIBD[2]);
	sigmaBias[2] = sqrt(sigmaBias[2] * sigmaBias[2] + (-0.05 * -0.05) + (0.09 * 0.09));
	
	   sigmaBias[0] = sigmaBias[0] * sqrt(effIBD[0]);
	   sigmaBias[1] = sigmaBias[1] * sqrt(effIBD[1]);
	sigmaBias[2] = sigmaBias[2] * sqrt(effIBD[2]);
	   ...
	   double tanPhiErrBias = sqrt( pow(sigmaBias[0]/sqrt(effIBD[0])*meanBias[1]/(meanBias[0]*meanBias[0]), 2) + pow(sigmaBias[1]/sqrt(effIBD[1])/(meanBias[0]), 2) );
	   ...
	   cout << "  Unbiased Tan(phi) error: " << tanPhiErrBias << "\n";
	   cout << "  Unbiased Tan(theta) error: " << tanThetaErrBias << "\n";
	}
	//----------------------------------------------------
