...
void fillDetectorConfig(vector<vector<int>>& detectorConfig){
    ...
    for (int i = 0; i < detectorConfig.size(); i++) {
        cout << "Detector configuration for period " << i << "\n";
...
int main(){
    ...
    cout << checkNeighbor(detectorConfig, 1, 39, 'r') << endl;
    cout << checkNeighbor(detectorConfig, 1, 43, 'r') << endl;
    cout << checkNeighbor(detectorConfig, 1, 91, 'r') << endl;
    ...
    for (vector<string>::iterator var = vars.begin(); var != vars.end(); ++var){
        ...
        h_data_CorOnBias.push_back ( new TH1D(Form("Data_CorrIBD_On_Bias%s",  var->c_str()), "Data", h_bins, -h_max, h_max) );
        h_data_CorOffBias.push_back( new TH1D(Form("Data_CorrIBD_Off_Bias%s", var->c_str()), "Data", h_bins, -h_max, h_max) );
        h_data_AccOnBias.push_back ( new TH1D(Form("Data_AccIBD_On_Bias%s",   var->c_str()), "Data", h_bins, -h_max, h_max) );
        h_data_AccOffBias.push_back( new TH1D(Form("Data_AccIBD_Off_Bias%s",  var->c_str()), "Data", h_bins, -h_max, h_max) );
        h_data_DiffBias.push_back  ( new TH1D(Form("Data_Diff_Bias%s",        var->c_str()), "Data", h_bins, -h_max, h_max) );
    ...
    //----------------------------------------------------
    {
       ...
       while( file.good() && !file.eof() ){
            ...
            if( countlines % 100 == 0 )
                cout << "Looking at Data file: " << countlines << " / 4052" << endl;
                    ...
                    if (pseg == nseg || diff_index > 0 || diff_index < 0){
                        ...
                        if( Esmear > 0.8 && Esmear < 7.2 && ncapt_dt > pow(10, 3) && ncapt_dt < 120 * pow(10, 3) ){
                            if( ReactorOn ){
                                ...
                                bool r = checkNeighbor(detectorConfig, periodNo, pseg, 'r');
                                bool l = checkNeighbor(detectorConfig, periodNo, pseg, 'l');
                                bool u = checkNeighbor(detectorConfig, periodNo, pseg, 'u');
                                bool d = checkNeighbor(detectorConfig, periodNo, pseg, 'd');
                        ...
                        if( Esmear > 0.8 && Esmear < 7.2 && ncapt_dt > pow(10, 6) ){
                            if( ReactorOn ){
       ...
       cout << " Total livetime for all RxOff " << livetimeOff << endl;
       cout << " Total livetime for all RxOn  " << livetimeOn  << endl;

       vector<double> mean, sigma, meanBias, sigmaBias;
       ...
       for (unsigned int ivar = 0; ivar != vars.size(); ++ivar)
       {
            ...
            h_data_Diff[ivar]->Add(h_data_CorOff[ivar], -livetimeOn/livetimeOff);
            h_data_Diff[ivar]->Add(h_data_AccOff[ivar], livetimeOn/livetimeOff/100.0);
            ...
            h_data_DiffBias[ivar]->Add(h_data_CorOffBias[ivar], -livetimeOn/livetimeOff);
            h_data_DiffBias[ivar]->Add(h_data_AccOffBias[ivar], livetimeOn/livetimeOff/100.0);
            ...
            cout << Form(" Total IBD events %0.0f ± %0.0f. Effective IBD counts %0.0f", totalIBDs, totalIBDsErr, effIBDs) << endl;
            ...
            if (ivar == 0 || ivar == 1){
                ...
                np = h_data_Diff[ivar]->GetBinContent(296);
                npp = h_data_DiffBias[ivar]->GetBinContent(296);
                nm = h_data_Diff[ivar]->GetBinContent(6);
                nmm = h_data_DiffBias[ivar]->GetBinContent(6);
                npm = h_data_DiffBias[ivar]->GetBinContent(151);
                n0 = h_data_Diff[ivar]->GetBinContent(151);

                nppErr = h_data_DiffBias[ivar]->GetBinError(296);
                npErr = h_data_Diff[ivar]->GetBinError(296);
                nmErr = h_data_Diff[ivar]->GetBinError(6);
                nmmErr = h_data_DiffBias[ivar]->GetBinError(6);
                npmErr = h_data_DiffBias[ivar]->GetBinError(151);
                n0Err = h_data_DiffBias[ivar]->GetBinError(151);
                ...
                pxErr = D * pow((1 / (rPlus + rMinus + 1)), 2) * sqrt((pow((2 * rMinus * rPlusErr + rPlusErr), 2) + (pow((2 * rPlus * rMinusErr + rMinusErr), 2))));
       ...
       cout << "  Mean x: "  << mean[0]  << ", Mean y: "  << mean[1]  << ", Mean z: "  << mean[2]  << endl;
       cout << "  Sigma x: " << sigma[0] << ", Sigma y: " << sigma[1] << ", Sigma z: " << sigma[2] << endl;
       cout << "  Largest sigma value: "    << lsigma           << endl;
       cout << "  Phi in degrees: "         << phiDeg_data      << endl;
       cout << "  Phi Error in degrees: "   << phiDegErr_data   << endl;
       cout << "  Theta in degrees: "       << thetaDeg_data    << endl;
       ...
       double phiRadErrBias = sqrt( pow(lsigmaBias/sqrt(effIBDs)*meanBias[1]/(meanBias[0]*meanBias[0]), 2) + pow(lsigmaBias/sqrt(effIBDs)/(meanBias[0]), 2) ) / (1 + pow(meanBias[1]/meanBias[0], 2)); 
       ...
       double thetaRadErrBias = sqrt( pow(lsigmaBias/sqrt(effIBDs)/sqrt(meanBias[0]*meanBias[0] + meanBias[1]*meanBias[1]), 2) * (pow(meanBias[0]*meanBias[2]/(meanBias[0]*meanBias[0] + meanBias[1]*meanBias[1]), 2) + pow(meanBias[1]*meanBias[2]/(meanBias[0]*meanBias[0] + meanBias[1]*meanBias[1]), 2) + 1) ) / (1 + pow(meanBias[2]/sqrt(meanBias[0]*meanBias[0] + meanBias[1]*meanBias[1]), 2));
       thetaDegErr_dataBias = thetaRadErrBias * 180.0/pi;

       cout << "  Mean Unbiased x: "  << meanBias[0]  << ", Mean Unbiased y: "  << meanBias[1]  << ", Mean Unbiased z: "  << meanBias[2]  << endl;
       cout << "  Sigma Unbiased x: " << sigmaBias[0] << ", Sigma Unbiased y: " << sigmaBias[1] << ", Sigma Unbiased z: " << sigmaBias[2] << endl;
       cout << "  Largest Unbiased sigma value: "    << lsigmaBias          << endl;
       cout << "  Unbiased Phi in degrees: "         << phiDeg_dataBias      << endl;
       cout << "  Unbiased Phi Error in degrees: "   << phiDegErr_dataBias   << endl;
       cout << "  Unbiased Theta in degrees: "       << thetaDeg_dataBias    << endl;
    ...
    //----------------------------------------------------
    {
       ...
       {
           countlines = countlines + 1;
           cout << "Looking at Realistic Simulation file " << countlines << " / 20" << endl;
           ...
           for( long i = 0; i < nentries; i++ )
           {
                ...
                for( unsigned int ivar = 0; ivar != vars.size(); ++ivar )
                {
                     ...
                     if( pseg == nseg || diff_index > 0 || diff_index < 0 )
                     {
                         // Correlated IBDs are IBDs where the neutron capture occurs between 1 and 120 us after the prompt signal
                         if( Esmear > 0.8 && Esmear < 7.2 && ncapt_dt > pow(10, 3) && ncapt_dt < 120 * pow(10, 3) )
                             h_realsim_Cor[ivar]->Fill(diff_index, xRx);
      
                         // Accidentals are IBDs with neutrons capturing over 1 ms after the prompt
                         if( Esmear > 0.8 && Esmear < 7.2 && ncapt_dt > pow(10, 6) )
                             h_realsim_Acc[ivar]->Fill(diff_index, xRx*xRx);
                     }
                     else if( pseg == nseg || diff_index > 0 || diff_index < 0 )
                     {
                         // Correlated IBDs are IBDs where the neutron capture occurs between 1 and 120 us after the prompt signal
                         if( Esmear > 0.8 && Esmear < 7.2 && ncapt_dt > pow(10, 3) && ncapt_dt < 120 * pow(10, 3) )
                             h_realsim_Cor[ivar]->Fill(diff_index, xRx);
       ...
       cout << "  Sigma x: " << sigma[0] << ", Sigma y: " << sigma[1] << ", Sigma z: " << sigma[2] << endl;
       cout << "  Largest sigma value: "    << lsigma              << endl;
       cout << "  Phi in degrees: "         << phiDeg_realsim      << endl;
       cout << "  Phi Error in degrees: "   << phiDegErr_realsim   << endl;
       cout << "  Theta in degrees: "       << thetaDeg_realsim    << endl;
    ...
    //----------------------------------------------------
    {
       ...
       cout << "  Sigma x: " << sigma[0] << ", Sigma y: " << sigma[1] << ", Sigma z: " << sigma[2] << endl;
       cout << "  Largest sigma value: "    << lsigma              << endl;
       cout << "  Phi in degrees: "         << phiDeg_perfsim      << endl;
       cout << "  Phi Error in degrees: "   << phiDegErr_perfsim   << endl;
       cout << "  Theta in degrees: "       << thetaDeg_perfsim    << endl;
    ...
    //----------------------------------------------------
    {
       ...
       cout << "Looking at True Reactor" << endl;
       cout << "  Phi in degrees: "         << phiDeg_true      << endl;
       cout << "  Phi Error in degrees: "   << phiDegErr_true   << endl;
       cout << "  Theta in degrees: "       << thetaDeg_true    << endl;
